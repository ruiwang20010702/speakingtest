...
export const RoadmapPage: React.FC = () => {
  ...
  return (
    <div className="w-full h-full flex flex-col relative bg-klein overflow-hidden select-none px-5 pt-6 pb-4">
      ...
      <div className="relative z-30 mb-3 flex-shrink-0">
        ...
        <motion.h2 
          ...
          className="text-2xl font-black text-white italic tracking-tighter mt-1.5 leading-none"
        >
      ...
      {/* 可滚动的内容区域 */}
      <div className="flex-1 overflow-y-auto relative z-20 space-y-3 pr-1">
        {/* 亮点和短板卡片 */}
        <motion.div 
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.2 }}
          className="relative bg-white border-2 border-black rounded-2xl px-4 pt-4 pb-4 shadow-[4px_4px_0px_#000] overflow-hidden"
        >
          <div className="absolute top-2 right-3 opacity-10">
             <Sparkles size={32} className="text-klein" />
          </div>
          ...
          <div className="space-y-3">
            ...
            {suggestion?.highlights && suggestion.highlights.length > 0 && (
              <div className="flex items-start space-x-2.5">
                <div className="w-7 h-7 rounded-lg bg-green-100 flex items-center justify-center flex-shrink-0 border border-green-200">
                  <CheckCircle size={14} className="text-green-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="text-[10px] font-black text-green-600 uppercase tracking-tight mb-1">亮点表现</h4>
                  <div className="space-y-0.5">
                    {suggestion.highlights.map((highlight, i) => (
                      <p key={i} className="text-[9px] font-bold text-gray-600 leading-snug">{highlight}</p>
                    ))}
                  </div>
                </div>
            </div>
            ...
            {suggestion?.weaknesses && suggestion.weaknesses.length > 0 && (
              <div className="flex items-start space-x-2.5">
                <div className="w-7 h-7 rounded-lg bg-orange-100 flex items-center justify-center flex-shrink-0 border border-orange-200">
                  <AlertTriangle size={14} className="text-orange-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="text-[10px] font-black text-orange-600 uppercase tracking-tight mb-1">提升空间</h4>
                  <div className="space-y-0.5">
                    {suggestion.weaknesses.map((weakness, i) => (
                      <p key={i} className="text-[9px] font-bold text-gray-600 leading-snug">{weakness}</p>
                    ))}
                  </div>
                </div>
            </div>
      ...
      {/* 底部 Footer - 固定在底部 */}
      <div className="mt-3 flex items-center justify-between opacity-30 px-2 flex-shrink-0 relative z-10">
