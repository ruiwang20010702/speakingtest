...
const HomePage: React.FC<HomePageProps> = ({ onStart }) => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center overflow-y-auto p-4 sm:p-6 pt-8 sm:pt-14 pb-8 sm:pb-12 relative bg-[#002FA7]">
      {step === 'intro' && (
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute top-[20%] right-[10%] animate-float opacity-10" style={{animationDelay: '1.5s'}}><Zap className="w-12 sm:w-16 h-12 sm:h-16 text-[#FFF59D] fill-current" /></div>
      ...
      <div className="w-full max-w-md flex justify-between items-center mb-6 z-10">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 sm:w-8 sm:h-8 bg-[#FFF59D] rounded-xl flex items-center justify-center">
            <Zap className="text-[#002FA7] w-4 h-4 sm:w-5 sm:h-5 fill-current" />
          </div>
          <span className="font-black text-lg sm:text-xl tracking-tight text-[#FFF59D]">51Talk AI</span>
        </div>
        ...
        <div className="flex-1 w-full max-w-md flex flex-col py-2 sm:py-4 z-10 min-h-0">
          <div className="flex flex-col items-center text-center space-y-2 mb-3 sm:mb-4 flex-shrink-0">
            ...
            <div className="space-y-1">
              <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">你好, 学习伙伴!</h1>
              <p className="text-sm sm:text-base text-white/80 font-bold">准备好开始你的口语测评了吗？</p>
            </div>
          </div>

          <div className="space-y-2.5 sm:space-y-3 pt-2 sm:pt-4 flex-1 flex flex-col justify-end pb-4">
            <div className="relative">
              <User className="absolute left-5 top-1/2 -translate-y-1/2 text-[#FFF59D] w-5 h-5" />
              <input 
                type="text" 
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="输入你的姓名"
                className="w-full bg-white/60 backdrop-blur-md rounded-[25px] border-2 border-transparent py-4 pl-14 pr-6 font-black text-lg text-[#1E293B] outline-none focus:border-[#FFF59D] focus:bg-white transition-all shadow-sm placeholder:text-gray-400"
              />
            </div>
            ...
            <div className="relative">
              ...
              <input 
                ...
                placeholder="输入手机号"
                maxLength={11}
                className="w-full bg-white/60 backdrop-blur-md rounded-[25px] border-2 border-transparent py-4 pl-14 pr-6 font-black text-lg text-[#1E293B] outline-none focus:border-[#58CC02] focus:bg-white transition-all shadow-sm placeholder:text-gray-400"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="relative">
                ...
                <select 
                  ...
                  className="w-full bg-white/60 backdrop-blur-md rounded-[25px] py-4 pl-12 pr-4 font-black text-[#1E293B] outline-none appearance-none cursor-pointer focus:bg-white"
                >
                  <option value="" disabled>选择等级</option>
                  {levels.map(l => (
                    <option key={l} value={l} className="text-[#1E293B]">{l} 等级</option>
                  ))}
                </select>
              </div>
              ...
              <div className={`relative transition-all duration-300 ${!level ? 'opacity-40' : 'opacity-100'}`}>
                ...
                <select 
                  ...
                  className="w-full bg-white/60 backdrop-blur-md rounded-[25px] py-4 pl-12 pr-4 font-black text-[#1E293B] outline-none appearance-none disabled:cursor-not-allowed disabled:text-gray-400"
                >
                  {!level && <option value="">待选等级</option>}
                  {level && !hasUnitDifference && <option value="Full Level" className="text-[#1E293B]">全单元</option>}
                  {level && hasUnitDifference && (
                    <>
                      <option value="Unit 1-4" className="text-[#1E293B]">Unit 1-4</option>
                      <option value="Unit 5-8" className="text-[#1E293B]">Unit 5-8</option>
                    </>
                  )}
                </select>
              </div>
            ...
            <button
              ...
              className={`w-full py-4 sm:py-5 rounded-[25px] font-black text-xl btn-duo flex items-center justify-center gap-3 mt-auto ${
                ...
                ? 'bg-[#FFF59D] text-[#002FA7] border-[#FBC02D]' 
                : 'bg-white/40 text-white/40 border-white/30 cursor-not-allowed'
              }`}
            >
        ...
        <div className="flex-1 w-full max-w-sm flex flex-col justify-between py-4 pb-8 animate-in fade-in slide-in-from-right-4 z-10">
          <div className="space-y-8">
            <div className="text-center space-y-2">
              <h2 className="text-3xl font-black text-white tracking-tight">测评任务清单</h2>
              <p className="text-white/80 font-bold text-base italic">Hi <span className="text-[#FFF59D]">{name}</span>, 我们将进行以下挑战:</p>
            </div>
            ...
            <div className="space-y-4">
              <div className="group relative bg-white/60 backdrop-blur-md p-5 rounded-[35px] border-2 border-white/50 flex items-center gap-5 shadow-lg hover:translate-y-[-2px] transition-all duration-300 overflow-hidden">
                ...
                <div>
                  <h4 className="font-black text-white text-lg">词汇挑战</h4>
                  <p className="text-white/70 text-sm font-bold">看图朗读核心单词</p>
                </div>
              </div>

              <div className="group relative bg-white/60 backdrop-blur-md p-5 rounded-[35px] border-2 border-white/50 flex items-center gap-5 shadow-lg hover:translate-y-[-2px] transition-all duration-300 overflow-hidden">
                <div className="absolute top-0 right-0 p-2 opacity-5 group-hover:opacity-10 transition-opacity">
                   <Zap className="w-16 h-16 text-[#FFF59D]" />
                </div>
                <div className="w-14 h-14 bg-[#FFF59D] rounded-[22px] flex items-center justify-center font-black text-[#002FA7] text-2xl shadow-inner">2</div>
                <div>
                  <h4 className="font-black text-white text-lg">回答老师的问题</h4>
                  <p className="text-white/70 text-sm font-bold">与 AI 进行 10-12 个对话</p>
                </div>
              </div>
            </div>

            <div className="bg-[#FFF59D]/10 backdrop-blur-sm p-5 rounded-[30px] border-2 border-[#FFF59D]/30 flex gap-4 items-center">
              <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm">
                <ShieldCheck className="w-7 h-7 text-[#FFF59D]" strokeWidth={2.5} />
              </div>
              <p className="text-[#FFF59D] text-xs font-black leading-snug">
          ...
          <div className="space-y-4 pt-8 pb-4">
            ...
            <button 
              ...
              className="w-full text-white/60 font-black text-sm text-center underline underline-offset-4 decoration-2 decoration-white/30"
            >
              返回修改信息
            </button>
          </div>
