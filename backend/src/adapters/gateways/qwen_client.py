...
## 评测理念
**你要评估的是学生的口语表达能力，而非"是否与参考答案一致"。**
1. **语义等价原则**：只要学生的回答在语义上正确回应了问题，即使表达方式与参考答案完全不同，也应给予认可。
2. **多元回答原则**：开放性问题（如喜好、描述、观点类）没有唯一正确答案，任何合理回答都应被接受。
3. **参考答案用途**：参考答案仅供你理解题目意图和常见回答方式，**绝不是评分的唯一标准**。
4. **能力导向**：评分应关注学生的语言能力（流利度、发音、词汇等），而非答案内容是否"标准"。
...
## 对齐与防串题规则 <alignment_rules>（必须严格遵守）
1. “停顿/嗯/重复/自我纠正”属于同一题的思考过程，不能因为停顿就切到下一题。
2. 只有当学生开始表达明显属于下一题的回答语义（或明确说出“下一题/Question X/第X题”）时，才可切换到下一题。
...
## 评测重点
1. **重点关注**：学生的口语能力（流利度、发音、词汇、句式）
2. **次要参考**：回答是否切题（注意：合理的不同回答都是正确的）
3. **不要**：将参考答案当作唯一标准，学生的表达可以有多种方式

## 要求
1. 输出整段逐字转写（保留原话，不要润色）
2. 给出 5 个维度的评分（0-100分）及总分
3. 对每题给出简短反馈（指出回答是否切题、主要语法错误等），无需单独打分
4. 给出 3-5 条针对 Part 2 问答表现的总体改进建议 (part2_overall_suggestion)
