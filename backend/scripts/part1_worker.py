...
async def handle_task(task: Part1Task) -> bool:
    ...
    if exceeded:
        logger.warning(f"Part 1 任务 {task.task_id} 超过最大重试次数 ({retry_count}/{MAX_RETRIES}) 或已完成，跳过处理")
        # 确保标记为失败
        try:
            async with AsyncSessionLocal() as db:
                await db.execute(
    ...
    try:
        logger.info(f"Worker 开始处理 Part 1 任务: {task.task_id} (重试 {retry_count}/{MAX_RETRIES})")
